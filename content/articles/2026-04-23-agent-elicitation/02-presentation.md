...
After slide 8, the layer stack appears as a **small icon in the top-right corner** of every kit-construction slide (slides 8 through 14). The icon shows which layers have been assembled so far. As the speaker progresses, the icon fills. When it's full, the prompt is built.
...
**Interactivity — the room votes, the speaker locks it in.** On each lever slide (9, 10, 11, 12, 13) the speaker invites the room to vote — a quick show of hands per level — then clicks the winning sentence (or arrow-keys to it and hits Enter). The selection is stored, the corner icon fills that layer, and the assembled-prompt slide (15) updates. Selections accumulate across the talk.

Layers 1 and 7 are not voted on — Layer 1 (Context) is set live by the speaker on slides 6, 7 and 8 (identity + task + intent), and Layer 7 (Output Shape) is locked in by the speaker on slide 14. The five middle layers are where the room collaborates.

By the time the room reaches slide 15, the audience sees the prompt *they collectively built* — with their fingerprints on every votable layer. They didn't have to type. They didn't have to take notes. They voted, watched it assemble, and now they own it. That is what they walk out with.

The lever slides start with no level pre-selected — the room makes a fresh choice on each lever rather than rubber-stamping a default.
...
| 5 | Your prompt has layers | 2 | first appearance, layer 1 outlined |
| 6 | Tell your agent who to be (Identity — Layer 1, sentence 1) | 2 | layer 1 (Context) active |
| 7 | Tell your agent what we're working on (Task — Layer 1, sentence 2) | 2 | layer 1 (Context) active |
| 8 | Tell your agent how to engage (Intent — Layer 1, sentence 3) | 2 | layer 1 (Context) active |
| 9 | Lever 1 — Output Constraint | 3 | layer 1 filled, layer 2 active → fills on vote |
| 10 | Lever 2 — Reflection Loop | 3 | layers 1–2 filled, layer 3 active → fills on vote |
| 11 | Lever 3 — Language Style | 3 | layers 1–3 filled, layer 4 active → fills on vote |
| 12 | Lever 4 — Exit Gate | 3 | layers 1–4 filled, layer 5 active → fills on vote |
| 13 | Lever 5 — Structure | 3 | layers 1–5 filled, layer 6 active → fills on vote |
| 14 | Name the form you want (Output Shape) | 4 | all 7 layers filled (icon complete) |
| 15 | What we built — *the assembled prompt* | 5 | — (icon retires; full prompt shown live) |
| 16 | Closing line — if your job is to listen | 5 | — |
| 17 | The AI we love | 5 | — |
...
> *"Grill me with questions until you understand the role, the team, and we together really understand how to attract the person we are looking for."*
...
### Slide 9 — Lever 1 — Output Constraint
...
### Slide 10 — Lever 2 — Reflection Loop
...
### Slide 11 — Lever 3 — Language Style
...
>
> ○  **L4 Conditional**
>    *"Speak like you would to a senior colleague or a client — personal but respectful of personal boundaries."*
>
> ○  **L5 Locked**
>    *"Use only strictly formal language in line with the standards of the current industry:"*
>
> *Strict*
...
>
> ✓  **L4 Conditional** ← *default*
>    *"Do not produce any output until you have asked me at least five clarifying questions about the situation."*
>
> ○  **L5 Locked**
...
>
> ○  **L3 Polite**
>    *"Make sure to cover the most important areas normally in a job listing."*
>
> ✓  **L4 Conditional** ← *default*
>    *"Cover all areas that are in job listings and make it complete."*
>
> ○  **L5 Locked**
>    *"Cover all areas present when writing a job listing — make sure to not miss any. I will tell you which parts are not needed."*
>
> *Strict*
...
### Slide 14 — Name the form you want

**On screen:**
> *Layout: same shape as slides 6–8 — handwritten principle on the left, copy block underneath. Corner icon: all 7 layers filled — the icon is now complete.*
>
> **Name the form you want.**
>
> *Plain language. As specific as you can. Name the form, the length, the sections, what to avoid.*
>
> ──────────
>
> **Like this →** *(Layer 7)*
...
**Speaker notes:**
One more layer. Tell the agent what shape the final output should take — before it produces a word.

A research brief reads differently from a one-pager. A list of interview questions reads differently from a candidate assessment. A performance review reads differently from a calibration note. The agent will produce whatever feels appropriate by default — and the default is rarely what you actually want.

So name it. Plain language, as specific as you can. Name the form, the length, the sections, what to avoid. The agent now knows exactly what it's aiming at.
...
### Slide 15 — What we built — *the assembled prompt*
...
>
> *[Layer 1, sentence 3 — the intent we set live, e.g. "Grill me with questions until you understand the role, the team, and we together really understand how to attract the person we are looking for."]*
>
...
The icon is roughly 60–80 pixels wide, positioned with a small margin from the top and right edges. It's reference, not headline. The audience picks it up by slide 5 without it being explained again.

After slide 14 the icon retires. Slides 15 onwards return to clean format.
...
- **Visual restraint.** Black backgrounds, white type, one idea per slide. Resist bullet lists except where the content is genuinely a list (the layers list on slide 5, the lever sentence menus on slides 9–13).
- **Lever slides (9–13).** Each is a single merged slide combining the lever's question, the vertical L1→L5 menu, and the voting interaction. The vertical layout reads top (Loose) to bottom (Strict). Each sentence sits beside an empty circle (○); the selected sentence shows a checkmark (✓). On entry the cursor sits on L4 by default.
...
- **Pacing slides.** Black slides with a single sentence or short principle (Slides 2, 4, 16, 17) are breathing room — they let the room re-focus on the speaker, not the deck.
- **Closing rhythm — Slides 16 → 17** is the two-beat landing: practical thesis followed by the emotional crescendo. Hold each one. Don't rush. They are the only quote-styled slides of the deck — save the visual treatment for them.
- **Slide 17 ("The AI we love")** is the emotional crescendo and the final slide. White type on deep black. Small space between the two lines. *The AI we love* in italic, slightly smaller than the line above. This is the slide they remember.
- **Slide 15** is the deck's quiet showpiece — the moment the assembled prompt appears, populated from selections. Treat it as a clean, scannable text block. The format should look exactly like what someone would paste into ChatGPT — because that's what it is. The Copy button sits inside the bottom-left of the prompt box; a small "Reset votes" link sits below the box for re-runs.
...
- *"Grill me with questions until you understand the role, the team, and we together really understand how to attract the person we are looking for."* — for hiring or briefing.
...
- **L4 — Conditional:** *"Speak like you would to a senior colleague or a client — personal but respectful of personal boundaries."*
- **L5 — Locked:** *"Use only strictly formal language in line with the standards of the current industry:"*
...
- **L4 — Conditional:** *"Do not produce any output until you have asked me at least five clarifying questions about the situation."*
...
- **L3 — Polite:** *"Make sure to cover the most important areas normally in a job listing."*
- **L4 — Conditional:** *"Cover all areas that are in job listings and make it complete."*
- **L5 — Locked:** *"Cover all areas present when writing a job listing — make sure to not miss any. I will tell you which parts are not needed."*
